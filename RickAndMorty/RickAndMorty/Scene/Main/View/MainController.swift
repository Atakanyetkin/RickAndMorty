...
class MainController: UIViewController, UIScrollViewDelegate {
    ...
    private func locationStageCallbacks() {
        viewModel.fetchingNextLocationsData.subscribe { value
            in
            if value {
                print("locations yakalamaya başlandı")
            } else {
        ...
        viewModel.fetchingNextLocationsData.subscribe { _ in
            print("locations yakalama başarılır")
        }.disposed(by: disposeBag)
        
        viewModel.errorMsg.subscribe { errorMsg in print(errorMsg)
